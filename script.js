...
const cryptoUtils = {
    ...
    deriveKey: async (passphrase, salt) => {
        const keyMaterial = await crypto.subtle.importKey('raw', cryptoUtils.stringToArrayBuffer(passphrase), { name: 'PBKDF2' }, false, ['deriveKey']);
        return crypto.subtle.deriveKey({ name: 'PBKDF2', salt, iterations: 250000, hash: 'SHA-256' }, keyMaterial, { name: 'AES-GCM', length: 256 }, true, ['encrypt', 'decrypt']);
    },
    encryptMessage: async (message, passphrase) => {
        try {
            ...
            const encrypted = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, compressed || cryptoUtils.stringToArrayBuffer(message));
    ...
    decryptMessage: async (encryptedBase64, passphrase) => {
        try {
            ...
            return cryptoUtils.arrayBufferToString(ciphertext.length > 100 ? pako.inflate(new Uint8Array(decrypted)) : new Uint8Array(decrypted));
...
const ui = {
    displayMessage: (content, isSent = false) => {
        ...
        if (!isSent) elements.messagesDiv.querySelector('.message-placeholder')?.remove();
    ...
    generateQR: async (data) => {
        return new Promise((resolve, reject) => {
            ...
            QRCode.toCanvas(elements.qrCanvas, data, { width: size, margin: 2, color: { dark: '#000000', light: '#ffffff' } }, (error) => {
    ...
    showLoader: (button, text = 'Processing...') => { button.innerHTML = `<i class="fas fa-spinner fa-spin"></i> ${text}`; button.disabled = true; },
    resetButton: (button, originalHTML) => { button.innerHTML = originalHTML; button.disabled = false; },
    showError: (message) => {
...
const handlers = {
    handleEncrypt: async () => {
        ...
        if (!message || !passphrase) { ui.showError('Please enter both a message and passphrase'); return; }
        if (passphrase.length < 8) { ui.showError('Passphrase must be at least 8 characters long'); return; }
        ...
        } catch (error) { console.error('Encryption error:', error); ui.showError(error.message); }
        ui.resetButton(elements.sendButton, `<i class="fas fa-lock"></i> Encrypt & Generate QR`);
    },
    handleDecrypt: async () => {
        const file = elements.qrUpload.files[0];
        const passphrase = elements.passphraseInput.value.trim();
        if (!file || !passphrase) { ui.showError('Please select a QR file and enter passphrase'); return; }
        ...
        } catch (error) { console.error('Decryption error:', error); ui.showError(error.message.includes('decrypt') ? 'Decryption failed. Wrong passphrase?' : error.message); }
